...
import { db } from "./db";
import { eq, desc, and, or, count, sql, gte, lt, asc, inArray, not } from "drizzle-orm";
...
export class DatabaseStorage implements IStorage {
  ...
  async getRecentActivity(userId: string, limit: number = 10): Promise<{
    ...
    for (const task of recentTasks) {
      // Use updatedBy to show who actually updated the task, not the assignee
      const updater = task.updatedBy ? await this.getUser(task.updatedBy) : null;
  ...
  async getTodaysMeetings(userId: string): Promise<(Meeting & { 
    ...
    // Get today's meetings (excluding voice recordings)
    const todaysMeetings = await db
      ...
      .where(
        and(
          ...
          // Exclude voice recordings from Today's Schedule
          not(sql`${meetings.title} LIKE '%Voice Recording%'`),
          not(sql`${meetings.title} LIKE '%from Modal%'`),
          not(sql`${meetings.description} LIKE '%AI-processed%'`)
        )
      )
